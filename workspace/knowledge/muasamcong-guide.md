...
## MCP Server — Hướng dẫn cho Agent

### Tại sao dùng MCP thay browser?

MSC cung cấp API JSON trực tiếp — không cần browser headless. MCP server gọi API nhanh hơn, ổn định hơn, và không phụ thuộc Chromium/Playwright.

### MCP tools

| Tool | Mô tả |
| --- | --- |
| `mcp_fetch` | Gọi HTTP request (GET/POST), trả về response body |
| `web_search` | Tìm kiếm web (Brave Search) |
...
```
1. mcp_fetch → POST https://muasamcong.mpi.gov.vn/o/egp-portal-notification-system/services/get-list
...
- Dùng `mcp_fetch` GET các URL trong bảng "URLs quan trọng" ở trên để đọc HTML
