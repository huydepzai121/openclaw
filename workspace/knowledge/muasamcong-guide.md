...
## API trực tiếp — Hướng dẫn cho Agent

### Tools sẵn có

Agent luôn có `exec` (https://docs.openclaw.ai/tools/exec) để chạy curl/shell commands và `web_fetch` (https://docs.openclaw.ai/tools/web) để GET endpoints trực tiếp — không cần cấu hình thêm, không cần quyền đặc biệt.

### Tại sao dùng API?

MSC cung cấp API JSON trực tiếp. Gọi bằng `exec curl` — nhanh và ổn định.

### API Endpoints

| Endpoint | Method | Mô tả |
| --- | --- | --- |
| `/o/egp-portal-notification-system/services/get-list` | POST | Lấy danh sách thông báo hệ thống |

### Recommended flow (PHẢI FOLLOW)

```
1. exec curl → POST https://muasamcong.mpi.gov.vn/o/egp-portal-notification-system/services/get-list
   Body: {"page": 1, "pageSize": 10}
   Headers: {"Content-Type": "application/json"}
   → Trả về JSON danh sách thông báo

2. Parse JSON response → extract tiêu đề, ngày, nội dung

3. web_search → "thông báo mới site:muasamcong.mpi.gov.vn"
   → Bổ sung thông tin từ kết quả search (nếu cần)
```

### Lưu ý quan trọng

- API trả về JSON — parse trực tiếp, không cần DOM snapshot
- Nếu API trả lỗi 401/403 → endpoint có thể đã thêm auth, báo lại cho user
- Nếu API thay đổi schema → kiểm tra response structure trước khi parse
- Luôn ghi rõ **ngày** và **nguồn** khi tổng hợp kết quả

### Fallback nếu API fail

Nếu API không khả dụng:

- Dùng `web_search` với query: `"thông báo" site:muasamcong.mpi.gov.vn`
- Dùng `exec curl` GET các URL trong bảng "URLs quan trọng" ở trên để đọc HTML
